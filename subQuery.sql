...
SELECT salary from employees1 WHERE department_name = 'HR'; -- 63000

-- //! SELECT * from employees WHERE salary > 63000

-- Can return a single value - select clause
SELECT * from employees1 WHERE salary > (SELECT max(salary) from employees1 WHERE department_name = 'HR')

-- Can return multiple rows - from clause
SELECT * FROM (SELECT department_name, SUM(salary) FROM employees1 GROUP BY department_name) as sum_dept_salary

-- Can return a single column - where clause
